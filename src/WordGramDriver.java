...
public class WordGramDriver {
	public final static int WSIZE = 6;

// BENCHMARK
	public static int benchmark(Set<WordGram> set, String filename) throws FileNotFoundException {
		Scanner scan = new Scanner(new File(filename));

		ArrayList<String> list = new ArrayList<>(); //ArrayList created with all words in file
		while (scan.hasNext()) {
			String s = scan.next();
			list.add(s);
		}
		scan.close();
		String[] words = list.toArray(new String[0]); //Array created from ArrayList
	...
	public static int benchmarkShift(Set<WordGram> set, String filename) throws FileNotFoundException {
		...
		String[] words = new String[WSIZE]; //Array of size WSIZE created
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		System.out.printf("total # wgs = %d\n",total);
		//CALCULATE THE DIFFERENCE :)
		System.out.println(total-hset2.size());
